...
export function useStoredTaskDurations(
  ...
  }>({ sessionKey: null, durations: {} });

  useEffect(() => {
    if (ownerId === undefined || sessionKey === null) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setSnapshot({ sessionKey: null, durations: {} });
      return;
    }
    const next = Object.fromEntries(
      tasks.flatMap((task) => {
        const duration = readTaskWorkspaceDuration(ownerId, task, practiceId);
        return duration === null ? [] : [[task.id, duration]];
      }),
    );
    // Session storage is only available after hydration.
    setSnapshot({ sessionKey, durations: next });
  }, [ownerId, practiceId, refreshToken, sessionKey, tasks]);

  return snapshot.sessionKey === sessionKey ? snapshot.durations : {};
}
...
function readTaskWorkspaceDuration(
  ...
  try {
    return parseTaskWorkspaceDuration(
      sessionStorage.getItem(taskDraftStorageKey(ownerId, task.id, practiceId)),
      task.partCount,
      task.maxHints,
    );
  } catch {
