...
-- name: GetPrepPreferences :one
select *
from prep_preferences
where user_id = sqlc.arg(user_id);

-- name: CreatePrepPreferences :one
insert into prep_preferences (user_id, goal_points, exam_date)
values (sqlc.arg(user_id), sqlc.arg(goal_points), sqlc.arg(exam_date))
on conflict (user_id) do nothing
returning *;

-- name: UpdatePrepPreferences :one
