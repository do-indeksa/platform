...
func TestLimitInFlightRejectsExcessAndReleasesSlot(t *testing.T) {
	...
	handler.ServeHTTP(overloadResponse, httptest.NewRequest(http.MethodGet, "/overload", nil))
	...
	handler.ServeHTTP(afterReleaseResponse, httptest.NewRequest(http.MethodGet, "/after-release", nil))
...
func TestLimitInFlightRejectsNonPositiveLimit(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatal("LimitInFlight(0) did not panic")
		}
	}()
