...
func TestProductionMiddlewareLogsOnlyAllowlistedRequestMetadata(t *testing.T) {
	var output bytes.Buffer
	logger := slog.New(slog.NewJSONHandler(&output, nil))
	router := loggedTestRouter(logger)
	...
	responseIDs := make(map[string]struct{}, len(requests))
	for _, testRequest := range requests {
		...
		if _, duplicate := responseIDs[requestID]; duplicate {
			t.Fatalf("duplicate response request ID %q", requestID)
		}
		responseIDs[requestID] = struct{}{}
	}
	...
	assertLogKeys(t, entries[0], "time", "level", "msg", "request_id", "method", "route", "status", "bytes", "duration_ms")
	assertLogKeys(t, entries[1], "time", "level", "msg", "request_id", "method", "route", "status", "bytes", "duration_ms")
	assertLogKeys(t, entries[2], "time", "level", "msg", "request_id", "method", "route", "status", "bytes", "duration_ms")
	assertLogKeys(t, entries[3], "time", "level", "msg", "request_id", "method", "route", "status", "bytes", "duration_ms")
	assertLogKeys(t, entries[4], "time", "level", "msg", "request_id", "method", "route", "status", "bytes", "duration_ms")
	...
	assertLogKeys(t, entries[6], "time", "level", "msg", "request_id", "method", "route", "status", "bytes", "duration_ms")
}

func TestProductionMiddlewareNormalizesUnknownMethods(t *testing.T) {
	var output bytes.Buffer
...
func loggedTestRouter(logger *slog.Logger) *chi.Mux {
	configureRequestLogging(logger)
	router := chi.NewRouter()
	router.Use(httpx.ServerRequestID)
	router.Use(middleware.RequestID, middleware.Logger, middleware.Recoverer, middleware.NoCache)
	return router
}
