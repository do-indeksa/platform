...
func TestApplicationLoggerWritesJSON(t *testing.T) {
	var output bytes.Buffer
	newApplicationLogger(&output).Info("application event", "count", 1)

	entries := decodeLogEntries(t, output.String())
	if len(entries) != 1 {
		t.Fatalf("got %d log entries, want one", len(entries))
	}
	assertLogKeys(t, entries[0], "time", "level", "msg", "count")
	if entries[0]["msg"] != "application event" || entries[0]["count"] != float64(1) {
		t.Fatalf("unexpected application log entry: %#v", entries[0])
	}
}
...
func TestRequestLoggingUsesOnlyAllowlistedMetadata(t *testing.T) {
	...
	for _, requestSpec := range requests {
		request := httptest.NewRequest(requestSpec.method, requestSpec.target, strings.NewReader(requestSpec.body))
		...
		request.Header.Set(httpx.RequestIDHeader, "caller-request-id-secret")
		response := httptest.NewRecorder()

		router.ServeHTTP(response, request)

		if response.Code != requestSpec.status {
			t.Fatalf("%s %s returned %d, want %d", requestSpec.method, requestSpec.target, response.Code, requestSpec.status)
		}
	...
	for index, entry := range entries[:5] {
		assertLogKeys(t, entry,
			"time", "level", "msg", "request_id", "method", "route", "status", "bytes", "duration_ms",
		)
		assertRequestID(t, entry, responseIDs[index])
	}
	if entries[5]["msg"] != "http panic recovered" {
		t.Fatalf("recovery message = %#v, want http panic recovered", entries[5]["msg"])
	}
...
func TestRequestLoggingNormalizesUnknownMethods(t *testing.T) {
	...
	entries := decodeLogEntries(t, logs)
	if len(entries) != 1 {
		t.Fatalf("got %d log entries, want one: %s", len(entries), logs)
	}
...
func loggedTestRouter(logger *slog.Logger) *chi.Mux {
	...
	router.Use(httpx.RequestID, httpx.RequestLogger(logger), middleware.Recoverer)
	return router
}
