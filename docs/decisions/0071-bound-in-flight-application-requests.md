# 0071 - Bound in-flight application requests per process
...
no-cache, security-header, and unsafe-origin middleware. A request that cannot
immediately acquire a slot receives fixed JSON error code `server_busy`, HTTP
status `503`, and `Retry-After: 1`; it is not queued and does not enter the
application handler. A deferred release returns the slot after normal completion
or panic. `/healthz` and `/readyz` are registered outside the limiter.
