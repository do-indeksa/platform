# 0065 - Bound HTTP request execution and graceful shutdown

**Status:** accepted - 2026-08-17

## Context

The API already bounded request headers, bodies, GraphQL tokens and complexity,
and socket reads and writes. Those limits did not give handler code or its
database and OAuth calls an absolute request-context deadline. A valid request
could therefore leave cooperative downstream work alive until its own local
limit or connection failure.
...
The server retains a 30-second write timeout and now allows 30 seconds for
graceful shutdown. The lifecycle order is 20-second request execution below a
30-second socket write limit, with ten seconds between request cancellation and
shutdown expiry.

Current PostgreSQL and provider calls inherit the request context. Handlers keep
...
The private Kubernetes API workload must set `terminationGracePeriodSeconds`
above 30 seconds; the deployment contract requires at least 35 seconds so the
platform does not force-kill the process at the application deadline. This
decision does not mutate deployment manifests or live infrastructure.
