# 0058 - Separate API liveness from database readiness
...
traffic requires the database. Using that endpoint as a future Kubernetes
readiness probe could keep routing requests to a process that could answer HTTP
but could not serve the product.
...
`200`, restarts Postgres, and waits for readiness to recover. A future Kubernetes
readiness probe must set `timeoutSeconds` to at least three seconds so the
application's two-second dependency deadline owns the failure. No deployment
manifest or live infrastructure changes in this decision.
